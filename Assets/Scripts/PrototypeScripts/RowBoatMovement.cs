...
{
    public class RowBoatMovement : MonoBehaviour
    {
        //Drags
        ...
        void UserInput()
        {
            ...
            {
                WaterJetRotation_Y = waterJetTransform.localEulerAngles.y + 2f / 6;
        ...
        void UpdateWaterJet()
        {
            ...
            localPos.y = 0;
            localPos = water.GetLocalVertexPosition(localPos);
            Vector3 worldPos = water.transform.TransformPoint(localPos);
            var waveHeight = worldPos.y;

            if (waterJetTransform.position.y < waveHeight)
            {
